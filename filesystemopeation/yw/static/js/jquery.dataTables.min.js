...
(function(la, s, p) {
	(function(i) {
		...
		else jQuery && !jQuery.fn.dataTable && i(jQuery)
	})(function(i) {
		var l = function(h) {
			function n(a, b) {
				...
				if (a.aoPreSearchCols[d] ===
					p || a.aoPreSearchCols[d] === null) a.aoPreSearchCols[d] = i.extend({}, l.models.oSearch);
				else {
			...
			function ma(a, b, c, d) {
				...
				if (b)
					for (e = b.length - 1; e >= 0; e--) {
						...
						if (typeof m[f] === "number" && m[f] < 0) d(a.aoColumns.length + m[f], b[e]);
						else if (typeof m[f] ===
							"string") {
							j = 0;
							for (k = a.aoColumns.length; j < k; j++)
								if (m[f] == "_all" || i(a.aoColumns[j].nTh).hasClass(m[f])) d(j, b[e])
						}
					}
				if (c) {
					e = 0;
					...
					if (k || m || u || !x) {
						...
						for (b = a.aoData.length; g < b; g++) {
							...
							if (k && j.sType !=
								...
								if (B !== "") {
									B = G(B);
									if (j.sType === null) j.sType = B;
									else if (j.sType != B && j.sType != "html") j.sType = "string"
								}
							}
							if (j.mRender) y.innerHTML = F(a, g, c, "display");
							else if (j.mData !== c) y.innerHTML = F(a, g, c, "display");
							if (m) {
								B = da(a, g, c);
								y.innerHTML = B;
			...
			function Ka(a) {
				...
				if (a.oFeatures.bSort)
					for (b = 0; b < a.aoColumns.length; b++) a.aoColumns[b].bSortable !== false ? ya(a, a.aoColumns[b].nTh, b) : i(a.aoColumns[b].nTh).addClass(a.oClasses.sSortableNone);
			...
			function H(a) {
				...
				if (i.inArray(false, b) !== -1) P(a, false);
				else {
					var c, d;
					b = [];
					var e = 0,
					...
					if (a.bDeferLoading) {
						...
						c = a.oLanguage;
						f = c.sZeroRecords;
						if (a.iDraw == 1 && a.sAjaxSource !== null && !a.oFeatures.bServerSide) f = c.sLoadingRecords;
			...
			function qa(a) {
				if (a.oFeatures.bSort) $(a, a.oPreviousSearch);
				else if (a.oFeatures.bFilter) X(a, a.oPreviousSearch);
				else {
					I(a);
					H(a)
				}
				...
				for (var c = a.nTableWrapper, d = a.sDom.split(""), e, f, g, j, k, m, u, x = 0; x < d.length; x++) {
					...
					if (g == "<") {
						...
						if (k == "'" || k == '"') {
							...
							if (m == "H") m = a.oClasses.sJUIHeader;
							else if (m == "F") m = a.oClasses.sJUIFooter;
							if (m.indexOf(".") != -1) {
								k =
									m.split(".");
								j.id = k[0].substr(1, k[0].length - 1);
								j.className = k[1]
							} else if (m.charAt(0) == "#") j.id = m.substr(1, m.length - 1);
							else j.className = m;
							x += u
						}
						c.appendChild(j);
			...
			function ra(a) {
				...
				if (a.bInitialised === false) setTimeout(function() {
					...
					for (c = a.aoColumns.length; b < c; b++)
						if (a.aoColumns[b].sWidth !== null) a.aoColumns[b].nTh.style.width = t(a.aoColumns[b].sWidth);
					if (a.oFeatures.bSort) $(a);
					else if (a.oFeatures.bFilter) X(a, a.oPreviousSearch);
					else {
						a.aiDisplay = a.aiDisplayMaster.slice();
						I(a);
			...
			function Na(a) {
				...
				// e.innerHTML = "<label>" + a.oLanguage.sLengthMenu.replace("_MENU_", b) + "</label>";注释entries
			...
			function Ga(a, b) {
				...
				if (b == "last")
					...
					else O(a, 0, "Unknown paging action: " + b);
			...
			function Qa(a) {
				...
				if (d.length > 0) {
					...
					else d._captionSide === "bottom" && u && k.appendChild(d)
				}
				if (a.oScroll.sX !== "") {
			...
			function $a(a) {
				...
				if (a.oScroll.sX === "") {
					...
					if (L && (i("tbody", d).height() > d.offsetHeight || i(d).css("overflow-y") == "scroll")) a.nTable.style.width = t(i(a.nTable).outerWidth() -
						a.oScroll.iBarWidth)
				} else if (a.oScroll.sXInner !== "") a.nTable.style.width = t(a.oScroll.sXInner);
				...
				if (i(a.nTable).outerWidth() < e) {
					...
					if (a.nTFoot !== null) a.nScrollFoot.style.width =
						t(g);
					if (a.oScroll.sX === "") O(a, 1, "The table cannot fit into the current element which will cause column misalignment. The table has been drawn at its minimum possible width.");
					else a.oScroll.sXInner !== "" && O(a, 1, "The table cannot fit into the current element which will cause column misalignment. Increase the sScrollXInner value or remove it to allow automatic calculation")
				} else {
					d.style.width = t("100%");
			...
			function ta(a) {
				...
				if (e == j.length && b === 0 && d == e && a.oScroll.sX === "" && a.oScroll.sY === "")
					for (f = 0; f < a.aoColumns.length; f++) {
						...
						for (f = d = 0; f < e; f++) {
							var m = a.aoColumns[f];
							if (m.bVisible && m.sWidthOrig !== null && m.sWidthOrig !== "") j[f - d].style.width = t(m.sWidthOrig);
							else if (m.bVisible) j[f - d].style.width = "";
							else d++
						}
						for (f =
							0; f < e; f++)
						...
						g.appendChild(b);
						if (a.oScroll.sX !== "" && a.oScroll.sXInner !== "") b.style.width = t(a.oScroll.sXInner);
						else if (a.oScroll.sX !== "") {
							b.style.width = "";
							if (i(b).width() < g.offsetWidth) b.style.width = t(g.offsetWidth)
						} else if (a.oScroll.sY !== "") b.style.width = t(g.offsetWidth);
						else if (k) b.style.width = t(k);
			...
			function $(a, b) {
				...
				// 		} else b.setAttribute("aria-label", e + (y[c].asSorting[0] == "asc" ? B.sSortAscending : B.sSortDescending));
				// 		else b.setAttribute("aria-label", e)
				// }
				a.bSorted = true;
				i(a.oInstance).trigger("sort", a);
				if (a.oFeatures.bFilter) X(a, a.oPreviousSearch, 1);
				else {
					a.aiDisplay =
			...
			function ba(a) {
				...
				if (a.oFeatures.bSort && a.oFeatures.bSortClasses) {
					...
					for (c = a.length; b < c; b++) {
						...
						k = e[j];
						j = d.replace(f, k);
						if (j != d) a[b].className = i.trim(j);
						else if (k.length > 0 && d.indexOf(k) == -1) a[b].className = d + " " + k
					}
				}
			}
			...
			function O(a, b, c) {
				...
				if (b === 0)
					...
					else la.console && console.log && console.log(a)
			}
			...
			function hb(a, b) {
				...
				for (var d in b)
					if (b.hasOwnProperty(d)) {
						c = b[d];
						if (typeof h[d] === "object" && c !== null && i.isArray(c) === false) i.extend(true, a[d], c);
						else a[d] = c
					}
				return a
			}
			...
			this.fnDeleteRow = function(a, b, c) {
				...
				for (f = d.aoData.length; e < f; e++)
					if (d.aoData[e].nTr !== null) d.aoData[e].nTr._DT_RowIndex =
						e;
				...
				if (g[a].bVisible != b) {
					if (b) {
						...
						for (f = j.length; e < f; e++)
							if (j[e].nTr !== null) m ? j[e].nTr.appendChild(j[e]._anHidden[a]) : j[e].nTr.insertBefore(j[e]._anHidden[a], W(d, e)[k])
					} else {
						e = 0;
						for (f = j.length; e < f; e++)
							if (j[e].nTr !== null) {
			...
			this.each(function() {
				...
				var e = false,
					f = false;
				if (this.nodeName.toLowerCase() != "table") O(null, 0, "Attempted to initialise DataTables on a node which is not a table: " + this.nodeName);
				else {
					a = 0;
					...
					if (g.oFeatures.bServerSide && g.oFeatures.bSort && g.oFeatures.bSortClasses) J(g, "aoDrawCallback", ba, "server_side_sort_classes");
					else g.oFeatures.bDeferRender && J(g, "aoDrawCallback", ba, "defer_sort_classes"); if (h.bJQueryUI) {
					...
					if (h.oLanguage.sUrl !== "") {
						...
						e = true
					} else i.extend(true, g.oLanguage, h.oLanguage); if (h.asStripeClasses === null) g.asStripeClasses = [g.oClasses.sStripeOdd, g.oClasses.sStripeEven];
					...
					if (f)
						for (a = 0; a < h.aaData.length; a++) R(g, h.aaData[a]);
					else ea(g);
			...
			full_numbers: {
				...
				fnUpdate: function(h, n) {
					if (h.aanFeatures.p) {
						...
						if (h._iDisplayLength === -1) w = o = A = 1;
						else if (v < q) {
							A = 1;
							o = v
						} else if (w <= o) {
